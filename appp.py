...
# @app.route('/product_info/<int:student_id>')
# def product_info(student_id):
#     # MySQL 데이터베이스 연결
#     conn = pymysql.connect(**db_config)
#     cursor = conn.cursor()
...
#     # 데이터를 HTML 템플릿으로 전달
#     return render_template('product_info.html', student=student)
...
def display_promotion_product():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if event_product:
                cursor.execute("INSERT INTO cart (cart_id, customer_id, product_id, name, price) VALUES (%s, %s, %s, %s, %s)",
                               ('241127-001', '15-3215', event_product[0], event_product[1], event_product[2]))
                conn.commit()
...
def display_cart():
    ...
    if request.method == 'POST':
        ...
        if action == 'delete':
            ...
            try:
                ...
                if product:
                    cursor.execute("INSERT INTO save_event_product (customer_id, product_id, name, quantity) VALUES (%s, %s, %s, %s)", 
                                   (product[1], product[2], product[3], quantity))
                    cursor.execute("INSERT INTO payment (customer_id, product_id, payment_date, product_name, quantity, total_payment) VALUES (%s, %s, %s, %s, %s, %s)", 
                                   (product[1], product[2], '2024-11-29', product[3], quantity, product[4] * quantity))
                    conn.commit()
    ...
    # cart 테이블과 event_product 테이블 조인
    cursor.execute("""SELECT cart.cart_id, cart.customer_id, cart.product_id, cart.name, cart.price, 
                   event_product.event_description, event_product.duration 
                   FROM cart 
                   INNER JOIN event_product 
                   ON cart.product_id = event_product.product_id""")
    # 조인된 데이터 가져오기
